...
```typescript
await page.fill('#id_login', username);  // ← FAILS HERE
```
...
**Root Issue**: Cannot log in → All authenticated tests fail
...
**Example Failures**:
```
- signup form: Expected 'form.signup' or 'form#signup_form' → not found
...
curl http://localhost:7600/accounts/login/
